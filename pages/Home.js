...
export default function Home() {
  ...
  useEffect(()=>{
    fetch("http://localhost:3000/events").then((response)=> response.json()).then((data)=> setEvents(data))
    fetch("http://localhost:5000/eventcategories").then((response)=> response.json()).then((data)=> setCategories(data))
    fetch("http://localhost:3000/home_banners").then((response)=> response.json()).then((data)=> setHomeBanners(data))
  }, [])
  return (
    <div>
      <Navbar/>
      ...
      <Row>
        <Col span={24}>
          <div
            style={{ fontWeight: "bolder", marginLeft: 40, fontSize: "30px" }}
          >
